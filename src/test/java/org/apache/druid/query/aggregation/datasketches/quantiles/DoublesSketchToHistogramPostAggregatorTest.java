...
public class DoublesSketchToHistogramPostAggregatorTest
{
  ...
  public void moreThan50Bins()
  {
    ...
    final Aggregator agg = new DoublesSketchBuildAggregator(selector, 32768);
    //noinspection ForLoopReplaceableByForEach
    for (int i = 0; i < values.length; i++) {
      agg.aggregate();
      selector.increment();
    ...
    DoublesSketch sketch = DoublesSketchOperations.deserialize(((DoublesSketch) agg.get()).toByteArray(true));
    final Map<String, Object> fields = new HashMap<>();
